...
export class MountingHud {
    ...
    static addButton(app: TokenHUD, html: JQuery, hudToken: hudToken) {
        ...
        if (hasProperty(selectedToken?.document, `flags.${MODULE_ID}.mount_id`)) {
            riderToken = selectedToken;
            mountToken = getToken(
                // @ts-ignore
                selectedToken?.document.getFlag(MODULE_ID, 'mount_id'),
            );
            mounted = true;
        } else if (hasProperty(selectedToken?.document, `flags.${MODULE_ID}.rider_id`)) {
            riderToken = getToken(
                // @ts-ignore
                selectedToken?.document.getFlag(MODULE_ID, 'rider_id'),
            );
        ...
        button.find('i').on('click', async function (event) {
            if (riderToken?.document.getFlag(MODULE_ID, 'mount_id') != undefined) {
                // Attempt to dismount...
